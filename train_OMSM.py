...
from diffusers import EulerDiscreteScheduler, UNetSpatioTemporalConditionModel
...
from flovd.models.flow_generator import FlowGenerator, get_flow_generator_input, flow_vis
from flovd.modules.utils import warp_image
...
def save_dict(batch, conditioning_images, sample, log_dict, output_dir, global_step, idx, r_type):
    ...
    sample_vis = flow_vis(sample, clip_flow=max(sample.shape[-2:])).clip(0., 1.)
    sample_vis = rearrange(sample_vis, 'f c h w -> c f h w')  # [3, f 2h, w]
    # pixel_values = rearrange(batch['pixel_values'].cpu(), '(b f) c h w -> b f c h w', b=1)
    pixel_values = rearrange(batch['pixel_values'][0] / 2. + 0.5, 'f c h w -> c f h w').detach().cpu()
    ...
    if 'clip_name' in batch:
        gif_path = os.path.join(gif_save_path_root, f"batch-{idx:04d}_{'_'.join(batch['video_caption'][0].split(' ')[:10])}_{batch['clip_name'][0]}_Sample.gif")
        video_path = os.path.join(gif_save_path_root, f"batch-{idx:04d}_{'_'.join(batch['video_caption'][0].split(' ')[:10])}_{batch['clip_name'][0]}_video.gif")
        ...
        gif_path = os.path.join(gif_save_path_root, f"batch-{idx:04d}_{'_'.join(batch['video_caption'][0].split(' ')[:10])}_Sample.gif")
        video_path = os.path.join(gif_save_path_root, f"batch-{idx:04d}_{'_'.join(batch['video_caption'][0].split(' ')[:10])}_video.gif")
        grid_path = os.path.join(grid_save_path_root, f"batch-{idx:04d}_{'_'.join(batch['video_caption'][0].split(' ')[:10])}_cond_image.png")
        
    save_videos_grid(sample_vis[None, ...], gif_path, rescale=False)
    save_videos_grid(pixel_values[None, ...], video_path, rescale=False)
    torchvision.utils.save_image(conditioning_image, grid_path)
    
    # Save flow warped video
    flow_pred = log_dict['sample_orig'][0]
    gif_path = os.path.join(gif_save_path_root, f"batch-{idx:04d}_{'_'.join(batch['video_caption'][0].split(' ')[:10])}_predicted_flow_warped_image.gif")
    flow_warped_image = warp_image(repeat(conditioning_images / 2. + 0.5, "b c h w -> (b f) c h w", f=flow_pred.shape[0]), flow_pred)
    flow_warped_image_gif = rearrange(flow_warped_image,'f c h w -> c f h w').to('cpu')  # [3, f 2h, w]
    save_videos_grid(flow_warped_image_gif[None, ...], gif_path, rescale=False)
    ...
    for key, value in log_dict.items():
        ...
        if key in ['observed_mask']:
            ...
            image_grid = torchvision.utils.make_grid(image_gif, nrow=14)
        elif key in ['sample_orig']:
            image_gif = flow_vis(value[0].cpu(), clip_flow=max(sample.shape[-2:])).clip(0.,1.)
            image_grid = torchvision.utils.make_grid(image_gif, nrow=14)
        else:
...
def main(name: str,
    ...
    *_, config = inspect.getargvalues(inspect.currentframe())
    ...
    vae = AutoencoderKLTemporalDecoder_Flow.from_pretrained(pretrained_model_path, subfolder="vae")
    unet = UNetSpatioTemporalConditionModel.from_pretrained(pretrained_model_path,
    ...
    runtime_type = ['training'] # No need to use depth warping?
    ...
    logger.info(f'Building Optimizer and Scheduler')
    trainable_params = [v for k, v in unet.named_parameters() if v.requires_grad and 'lora' not in k]
    trainable_param_names = [k for k, v in unet.named_parameters() if v.requires_grad and 'lora' not in k]
    ...
    # Validation pipeline
    validation_pipeline = StableVideoDiffusionPipelineOMSM(
        vae=vae,
    ...
    if resume_from is not None:
        ...
        unet.to('cpu')
        unet_m, unet_u = unet.module.load_state_dict(unet_state_dict, strict=False)
        assert len(unet_m) == 0 and len(unet_u) == 0
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step in range(trained_iterations, len(train_dataloader)):
            ...
            with torch.no_grad():
                pixel_values = rearrange(batch["pixel_values"].to(local_rank), "b f c h w -> (b f) c h w")  
            ...
            with torch.cuda.amp.autocast(enabled=mixed_precision_training, dtype=torch.bfloat16):
                with torch.no_grad():
                    flow_values, flow_log_dict = flow_generator(**flow_generator_input) # [b, f, c, h, w]
                    flow_values = flow_values.detach().clone()
                    # print("check the dimension of optical flow")
                    assert flow_values.ndim == 5 and flow_values.shape[2] == 2, "flow_values have to have 5-dim, and 2 channels."

            # Convert flow to latent space
            bsz, video_length = flow_values.shape[:2]
            with torch.no_grad():
                flow_values = rearrange(flow_values, "b f c h w -> (b f) c h w")
                if sample_latent:
                    latents = vae.encode_flow(flow_values).latent_dist.sample()
                else:
                    latents = vae.encode_flow(flow_values).latent_dist.mode()
            ...
            with torch.cuda.amp.autocast(enabled=mixed_precision_training, dtype=torch.bfloat16):
                model_pred = unet(input_latents,
                                  ...
                                  added_time_ids=add_time_ids).sample  # [b f c h w]
