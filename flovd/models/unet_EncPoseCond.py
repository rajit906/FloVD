...
from flovd.models.unet_3d_blocks import (
    get_down_block,
    get_up_block,
    UNetMidBlockSpatioTemporalPoseCond
)
from flovd.models.attention_processor import XFormersAttnProcessor as CustomizedXFormerAttnProcessor
from flovd.models.attention_processor import PoseAdaptorXFormersAttnProcessor

from flovd.models.attention_processor import PoseAdaptorAttnProcessor
from flovd.models.attention_processor import AttnProcessor as CustomizedAttnProcessor

import pdb

class UNetSpatioTemporalConditionModelEncPoseCond(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    r"""
    ...
    def set_pose_cond_attn_processor(self,
        ...
        if add_spatial:
            for processor_key in self.attn_processors.keys():
                ...
                if processor_key.startswith("down_block"):
                    # Add pose_adaptor only for Unet Encoder (down_block)
    ...
    def forward(
        ...
        for block_idx, downsample_block in enumerate(self.down_blocks):
            if hasattr(downsample_block, "has_cross_attention") and downsample_block.has_cross_attention:
                ...
                # add pose_features
                sample += rearrange(pose_features[block_idx], "b c t h w -> (b t) c h w")
