...
from flovd.models.motion_module import TemporalTransformerBlock
...
# Code based on PoseAdaptor from CameraCtrl (https://github.com/hehao13/CameraCtrl/tree/svd)
# Modified by Wonjoon Jin.
# Used in training FVSM
class FVSM_Adaptor(nn.Module):
    def __init__(self, unet, flow_encoder, flow_generator):
        super().__init__()
        self.unet = unet
        self.flow_encoder = flow_encoder
        self.flow_generator = flow_generator

    def forward(self, noisy_latents, c_noise, flow_generator_input, encoder_hidden_states, added_time_ids, use_encoder_inpainting=False):
        # 1) Get camera-object flow maps 
        cam_obj_flow, flow_log_dict = self.flow_generator(**flow_generator_input)
        
        # 2) Get multi-level flow embedding for the unet condition
        flow_embedding = self.flow_encoder(cam_obj_flow)      # b c f h w
        ...
        noise_pred = self.unet(noisy_latents,
                               ...
                               pose_features=flow_embedding).sample
